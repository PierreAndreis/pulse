...
8. As an app developer, I want to evolve my schema safely, so that I never lose data by accident. *(Shipped — two paths, Prisma/Drizzle-style: `pulse db push` syncs the schema to the DB with no files (additive auto-applies, destructive prompts or needs `--force`); `pulse migrate dev <name>` generates an **editable** `migrations/NNNN_<name>.sql` by diffing the schema against the last snapshot (no DB needed), with destructive drops commented out, then applies pending migrations; `pulse migrate deploy` applies pending files in order via a `_pulse_migrations` journal (idempotent, hash-drift-guarded); `pulse migrate status` reports state. See `docs/MIGRATIONS.md`.)*
