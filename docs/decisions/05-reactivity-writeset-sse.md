...
> **Update — several items this ADR deferred have since landed (see ADR 09 & 10):** multi-instance reactor state and reconnection (the cross-node interest-routed bus), cross-client dedup of identical subscriptions, key/filter-level matching, whole-value diffing, and a (sampled) `commitLsn` on pushes. Still genuinely deferred: SSE `Last-Event-ID` resume, client LSN batch-advance, raw-SQL capture, the `pulse-sse` crate extraction, and a per-subscription debounce window. The body below is left as the point-in-time M2 record.
