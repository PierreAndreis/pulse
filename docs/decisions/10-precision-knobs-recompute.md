...
- **Update (since landed):** the tractable subset of the recompute frontier this ADR identified is now implemented — **incremental view maintenance for `count(*)`, `sum`, and `min`/`max` (over both `int8` and `double precision` fields)** (the reactor maintains the scalar from the change delta with zero worker re-execs; see the IVM section under "Out of Scope / Deferred") and **batched `get`s / DataLoader** (the worker collapses concurrent per-row `get`s into one `ANY($1::uuid[])` round-trip). `count(distinct)` remains deferred (`avg` now landed — the engine seeds the reactor with the sum+count behind it).
...
- **Incremental view maintenance** — **`count(*)`, `sum`, and `min`/`max` (over `int8` *and* `double precision` fields) SHIPPED**: the reactor maintains the scalar from the change delta (+/− per row entering/leaving the filter, value-delta on a stay), with zero worker re-execs, falling back to re-execution for any shape it can't maintain exactly (jsonb fields aren't captured; an empty set's aggregate is `NULL`). `min`/`max` maintain a **rising extreme** incrementally — a row entering or growing to/past the current extreme (the common high-water-mark case) — but fall back when the row *holding* the extreme leaves or drops below it, since the next-best value isn't in the change image (the aggregate field is in `read_cols`, so such value-changes are never pruned). `avg` is also maintained (over int8/double): the engine seeds the reactor with the `sum`+`count` behind the avg (a `#[serde(skip)] avg_seed` on the read-set, computed alongside the avg in the same WHERE), and the reactor advances both by the change delta, recomputing `avg = sum/count` — falling back to a re-exec when the set empties (count → 0), which re-seeds cleanly. Proven by an `ivmApplied++` integration test.

  **The scalar-aggregate IVM frontier is complete** (`count(*)`, `sum`, `min`, `max`, `avg` — over both `int8` and `double precision`). **`count(distinct)` is deliberately NOT pursued** — it has no value sweet spot: maintaining it incrementally needs a per-value count map (++/−− on enter/leave; a value crossing 0↔1 moves the distinct total), which is only small for low-cardinality columns — but there the re-exec (a small `GROUP BY`) is already cheap, so IVM saves nothing; for high-cardinality columns, where a re-exec is expensive, the count map is impractically large. Since IVM exists to avoid *expensive* re-execs and `count(distinct)` has none it can cheaply maintain, it stays on the re-exec path. **Still genuinely deferred:** incremental **filtered-list** deltas (changed rows only, for live lists — the high-value next frontier, but a larger change than scalar aggregates).
  - **Field types:** `KeyValue` now carries a `Float(f64)` variant (hand-written `Eq`/`Hash`/`PartialEq` — float compares/hashes by raw bits — so it stays a sound `HashSet` key while never appearing in a real primary key). So both `v.int()` → `bigint` and `v.number()` → `double precision` field values are in the change image, and `sum`/`min`/`max` over either are maintained incrementally — proven end-to-end (integration tests assert `ivmApplied++` for a reactive `max(score:int)` *and* `max(qty:double)` on a new high) and by a property test over random change sequences for both paths. Fractional-float sums are inherently approximate (the re-exec is the source of truth and is itself order-dependent); `min`/`max` and integer-valued sums are exact. jsonb fields are still uncaptured → aggregates over them re-exec. `count(*)` is unaffected (no field).
