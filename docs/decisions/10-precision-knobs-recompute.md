...
- **Incremental view maintenance** — **`count(*)`, `sum(integer field)`, and `min`/`max(integer field)` SHIPPED**: the reactor maintains the scalar from the change delta (+/− per row entering/leaving the filter, value-delta on a stay), with zero worker re-execs, falling back to re-execution for any shape it can't maintain exactly (float/jsonb fields aren't captured in the change image; an empty set's aggregate is `NULL`). `min`/`max` maintain a **rising extreme** incrementally — a row entering or growing to/past the current extreme (the common high-water-mark case) — but fall back when the row *holding* the extreme leaves or drops below it, since the next-best value isn't in the change image (the aggregate field is in `read_cols`, so such value-changes are never pruned). **Deferred:** `avg` (needs auxiliary running sum+count state the initial query doesn't return), `count(distinct)` (needs the dataset, not just a delta), and incremental **filtered-list** deltas.
  - **Field-type caveat:** field aggregates (`sum`/`min`/`max`) maintain incrementally **only for integer (`v.int()` → `bigint`) fields**, because the change image (`RowValues`) carries no floats — `KeyValue` is `Hash + Eq` so it can back a `PrimaryKey` in a `HashSet`, and a float variant would break that. **`v.int()` columns ARE captured** (`int8 → KeyValue::Int`), so `sum`/`min`/`max` over them are maintained incrementally — proven end-to-end (an integration test asserts `ivmApplied++` for a reactive `max(score:int)` on a new high). `v.number()` → `double precision` values are *not* in the change image, so aggregates over them still re-exec. `count(*)` is unaffected (no field). Lifting the float case would need aggregate-field values decoupled from the `Hash + Eq` `KeyValue` (a separate float-capable image) — deferred alongside `avg`/`count(distinct)`.
