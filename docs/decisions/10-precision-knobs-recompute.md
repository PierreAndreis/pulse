...
- **Update (since landed):** the tractable subset of the recompute frontier this ADR identified is now implemented — **incremental view maintenance for `count(*)`, `sum`, and `min`/`max` (integer fields)** (the reactor maintains the scalar from the change delta with zero worker re-execs; see the IVM section under "Out of Scope / Deferred") and **batched `get`s / DataLoader** (the worker collapses concurrent per-row `get`s into one `ANY($1::uuid[])` round-trip). `avg` and `count(distinct)` remain deferred.
...
- **Incremental view maintenance** — **`count(*)`, `sum(integer field)`, and `min`/`max(integer field)` SHIPPED**: the reactor maintains the scalar from the change delta (+/− per row entering/leaving the filter, value-delta on a stay), with zero worker re-execs, falling back to re-execution for any shape it can't maintain exactly (float/jsonb fields aren't captured in the change image; an empty set's aggregate is `NULL`). `min`/`max` maintain a **rising extreme** incrementally — a row entering or growing to/past the current extreme (the common high-water-mark case) — but fall back when the row *holding* the extreme leaves or drops below it, since the next-best value isn't in the change image (the aggregate field is in `read_cols`, so such value-changes are never pruned). **Deferred:** `avg` (needs auxiliary running sum+count state the initial query doesn't return), `count(distinct)` (needs the dataset, not just a delta), and incremental **filtered-list** deltas.
  - **Dormancy caveat (important):** field aggregates (`sum`/`min`/`max`) maintain incrementally **only for integer fields**, because the change image (`RowValues`) carries no floats — `KeyValue` is `Hash + Eq` so it can back a `PrimaryKey` in a `HashSet`, and a float variant would break that. Today `v.number()` maps to `double precision`, so the only integer columns are system ones (`_creation_time`); over a typical numeric field these aggregates always fall back to re-exec. The IVM logic is correct and unit-tested (integer fields), but **dormant in practice** until either an integer column type exists or aggregate-field values are decoupled from the `Hash + Eq` `KeyValue` (carried in a separate float-capable image). `count(*)` is unaffected (no field). This — making field-aggregate IVM actually fire — is the genuine next step for the IVM line, ahead of `avg`/`count(distinct)`.
