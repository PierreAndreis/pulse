...
export const contract = {
  w: {
    ...
    sumScoreActive: numN(), // sum() over an int field + filter — maintained by IVM
    avgScoreActive: numN(), // avg() over an int field + filter — maintained from sum+count (IVM)
    // grouping
    byActive: rows(),
    sumByTag: rows(),
    ...
    addWidget: oc.mutation().input(v.object({ name: v.string(), qty: v.number(), price: v.optional(v.number()), score: v.optional(v.int()), active: v.boolean(), tag: v.optional(v.string()), authorId: v.optional(v.id("authors")), meta: v.optional(v.any()) })).output(v.any()),
